...
/// Pre-resolved addresses for building a Byreal CLMM swap instruction offline.
pub struct ByrealClmmSwapInput {
    pub payer: Address,
...
/// Build Byreal CLMM swap AccountMeta list from pre-resolved addresses (no RPC needed).
pub fn build_accounts(input: &ByrealClmmSwapInput) -> Vec<AccountMeta> {
    vec![
        AccountMeta::new_readonly(BYREAL_CLMM_PROGRAM_ID, false),
...
// Build Byreal CLMM swap extra data: [sqrt_price_limit_x64, is_base_input].
...
/// Resolve accounts and data for a Byreal CLMM swap via RPC.
///
