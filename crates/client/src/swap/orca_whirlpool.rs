...
// Whirlpool account data layout
// [8 discriminator] [32 whirlpools_config] [1 whirlpool_bump] [2 tick_spacing] [2 fee_tier_index_seed]
// [2 fee_rate] [2 protocol_fee_rate] [16 liquidity] [16 sqrt_price] [4 tick_current_index]
// [8 protocol_fee_owed_a] [8 protocol_fee_owed_b] [32 token_mint_a] [32 token_vault_a]
// [16 fee_growth_global_a] [32 token_mint_b]
#[cfg(feature = "resolve")]
const OFFSET_TICK_SPACING: usize = 41;
#[cfg(feature = "resolve")]
...
fn read_u16_le(data: &[u8], offset: usize) -> Result<u16, ClientError> {
    let end = offset + 2;
    if data.len() < end {
        return Err(ClientError::InvalidAccountData(
            "whirlpool data too short for spacing".into(),
        ));
    }
...
fn tick_array_pda(whirlpool: &Address, start_tick_index: i32) -> Address {
...
/// Pre-resolved addresses for building a Orca Whirlpool swap instruction offline.
pub struct OrcaWhirlpoolSwapInput {
...
/// Build Orca Whirlpool swap AccountMeta list from pre-resolved addresses (no RPC needed).
pub fn build_accounts(input: &OrcaWhirlpoolSwapInput) -> Vec<AccountMeta> {
    vec![
        ...
        AccountMeta::new(input.whirlpool, false),
...
/// Build Orca Whirlpool extra data: [sqrt_price_limit, amount_specified_is_input, a_to_b, remaining_accounts_info].
...
pub async fn resolve(
    ...
    let (pool_pubkey, pool_data) = match whirlpool {
        Some(addr) => {
    ...
    // `mint_a` / `mint_b` are input / output; `a_to_b` is whirlpool direction (A->B or B->A).
    ...
    let (oracle, _) =
        Address::find_program_address(&[b"oracle", pool_pubkey.as_ref()], &WHIRLPOOL_PROGRAM_ID);
    ...
    let input = OrcaWhirlpoolSwapInput {
        token_program_a,
        token_program_b,
