...
async fn test_perena_resolve_with_known_pair() {
    ...
    // Protocol program ID
    assert_eq!(accounts[0].pubkey, PERENA_PROGRAM_ID);
    assert!(!accounts[0].is_writable);

    // Pool
    assert_eq!(accounts[1].pubkey, USDC_USDG_STABLE_POOL);
    assert!(accounts[1].is_writable);

    // In mint
    assert_eq!(accounts[2].pubkey, USDC_MINT);
    assert!(accounts[2].is_writable);

    // Out mint
    assert_eq!(accounts[3].pubkey, USDG_MINT);
    ...
    assert_eq!(accounts[5].pubkey, expected_usdg_ata);
    assert!(accounts[5].is_writable);

    // In vault
    assert_eq!(accounts[6].pubkey, POOL_USDC_VAULT);
    assert!(accounts[6].is_writable);

    // Out vault
    assert_eq!(accounts[7].pubkey, POOL_USDG_VAULT);
    assert!(accounts[7].is_writable);

    // Numeraire config
    assert_eq!(accounts[8].pubkey, NUMERAIRE_CONFIG);
    ...
    // Token 2022 program
    assert_eq!(accounts[11].pubkey, TOKEN_2022_PROGRAM_ID);
...
async fn test_perena_resolve_flipped_mints() {
    ...
    // In mint
    assert_eq!(accounts[2].pubkey, USDG_MINT);
    assert!(accounts[2].is_writable);

    // Out mint
    assert_eq!(accounts[3].pubkey, USDC_MINT);
    assert!(accounts[3].is_writable);

    // In trader
    let expected_usdg_ata =
        beethoven_client::get_associated_token_address(&user, &USDG_MINT, &TOKEN_2022_PROGRAM_ID);
    assert_eq!(accounts[4].pubkey, expected_usdg_ata);
    assert!(accounts[4].is_writable);
    ...
    // In vault
    assert_eq!(accounts[6].pubkey, POOL_USDG_VAULT);
    assert!(accounts[6].is_writable);

    // Out vault
    assert_eq!(accounts[7].pubkey, POOL_USDC_VAULT);
    assert!(accounts[7].is_writable);
