...
pub struct CarrotBoost;

pub struct CarrotBoostDepositData {
    pub deposit_up_to_amount: u8,
}

impl CarrotBoostDepositData {
    pub const DATA_LEN: usize = 1;
}

impl TryFrom<&[u8]> for CarrotBoostDepositData {
...
pub struct CarrotBoostDepositAccounts<'info> {
...
impl<'info> TryFrom<&'info [AccountView]> for CarrotBoostDepositAccounts<'info> {
    type Error = ProgramError;
...
impl<'info> Deposit<'info> for CarrotBoost {
    type Accounts = CarrotBoostDepositAccounts<'info>;
    type Data = CarrotBoostDepositData;

    fn deposit_signed(
        ctx: &CarrotBoostDepositAccounts<'info>,
        amount: u64,
        data: &Self::Data,
        signer_seeds: &[Signer],
    ) -> ProgramResult {
        ...
        let deposit_ix = InstructionView {
            program_id: &CARROT_BOOST_PROGRAM_ID,
