use {
    crate::{AldrinV2, ALDRIN_V2_PROGRAM_ID, SWAP_DISCRIMINATOR},
...
pub struct AldrinV2SwapRemaining<'info> {
...
impl<'info> TryFrom<&'info [AccountView]> for AldrinV2SwapRemaining<'info> {
    ...
    fn try_from(accounts: &'info [AccountView]) -> Result<Self, Self::Error> {
        ...
        let [aldrin_v2_program, pool, pool_signer, pool_mint, base_token_vault, quote_token_vault, fee_pool_token_account, curve, token_program, ..] =
            accounts
        else {
            return Err(ProgramError::NotEnoughAccountKeys);
        };

        Ok(AldrinV2SwapRemaining {
            aldrin_v2_program,
            pool,
            pool_signer,
            pool_mint,
...
impl<'info> SwapWithParameters<'info> for AldrinV2 {
    type Remaining = AldrinV2SwapRemaining<'info>;
    type Extra = ();

    fn try_parse_remaining(
        remaining: &'info [AccountView],
    ) -> Result<Self::Remaining, ProgramError> {
        AldrinV2SwapRemaining::try_from(remaining)
    }
    ...
    fn swap_with_parameters_signed(
        ...
        let accounts = [
            ...
            InstructionAccount::readonly(remaining.curve.address()),
