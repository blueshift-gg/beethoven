...
pub const BYREAL_CLMM_PROGRAM_ID: Address = address!("REALQqNEomY6cQGZJUGwywTBD2UmDT32rZcNnfxQ5N2");

const SWAP_V2_DISCRIMINATOR: [u8; 8] = [43, 4, 237, 11, 26, 201, 30, 98];
const MAX_TICK_ARRAY_SIZE: usize = 16;
const MAX_ACCOUNTS: usize = ByrealClmmSwapAccounts::NUM_ACCOUNTS + MAX_TICK_ARRAY_SIZE;

pub struct ByrealClmm;

pub struct ByrealClmmSwapData {
    pub sqrt_price_limit_x64: u128,
    pub is_base_input: bool,
}

impl ByrealClmmSwapData {
    // 16 - sqrt_price_limit_x64
    // 1 - is_base_input
    pub const DATA_LEN: usize = 17;
}

impl TryFrom<&[u8]> for ByrealClmmSwapData {
...
impl ByrealClmmSwapAccounts<'_> {
    pub const NUM_ACCOUNTS: usize = 14;
}

pub struct ByrealClmmSwapAccounts<'info> {
    pub byreal_clmm_program: &'info AccountView,
...
impl<'info> TryFrom<&'info [AccountView]> for ByrealClmmSwapAccounts<'info> {
    type Error = ProgramError;

    fn try_from(accounts: &'info [AccountView]) -> Result<Self, Self::Error> {
        let [byreal_clmm_program, payer, amm_config, pool_state, input_token_account, output_token_account, input_vault, output_vault, observation_state, token_program, token_program_2022, memo_program, input_vault_mint, output_vault_mint, remaining_accounts @ ..] =
            accounts
        else {
            return Err(ProgramError::NotEnoughAccountKeys);
        };

        Ok(ByrealClmmSwapAccounts {
            byreal_clmm_program,
            payer,
            amm_config,
            pool_state,
...
impl<'info> Swap<'info> for ByrealClmm {
    type Accounts = ByrealClmmSwapAccounts<'info>;
    type Data = ByrealClmmSwapData;

    fn swap_signed(
        ctx: &Self::Accounts,
        in_amount: u64,
        minimum_out_amount: u64,
        data: &ByrealClmmSwapData,
        signer_seeds: &[Signer],
    ) -> ProgramResult {
