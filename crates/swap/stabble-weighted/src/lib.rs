...
pub const STABBLE_WEIGHTED_PROGRAM_ID: Address =
    address!("swapFpHZwjELNnjvThjajtiVmkz3yPQEHjLtka2fwHW");

const SWAP_V2_DISCRIMINATOR: [u8; 8] = [43, 4, 237, 11, 26, 201, 30, 98];

pub struct StabbleWeighted;

impl StabbleWeightedSwapAccounts<'_> {
    pub const NUM_ACCOUNTS: usize = 16;
}

pub struct StabbleWeightedSwapAccounts<'info> {
...
impl<'info> TryFrom<&'info [AccountView]> for StabbleWeightedSwapAccounts<'info> {
    ...
    fn try_from(accounts: &'info [AccountView]) -> Result<Self, Self::Error> {
        ...
        Ok(StabbleWeightedSwapAccounts {
            stabble_program,
            user,
            mint_in,
            mint_out,
...
impl<'info> Swap<'info> for StabbleWeighted {
    type Accounts = StabbleWeightedSwapAccounts<'info>;
    type Data = ();
    ...
    fn swap_signed(
        ...
        let instruction = InstructionView {
            program_id: &STABBLE_WEIGHTED_PROGRAM_ID,
            accounts: &accounts,
