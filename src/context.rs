...
pub enum SwapContext<'info> {
    ...
    HumaFinance(crate::huma_finance::HumaFinanceSwapAccounts<'info>),
}
...
impl<'a> SwapContext<'a> {
    ...
    pub fn try_from_swap_data(
        ...
        match self {
            ...
            #[cfg(feature = "huma-finance-swap")]
            SwapContext::HumaFinance(_) => Ok((SwapData::HumaFinance(()), data)),
...
impl<'a> Swap<'a> for SwapContext<'a> {
    ...
    fn swap_signed(
        ...
        match (ctx, data) {
            ...
            #[cfg(feature = "huma-finance-swap")]
            (SwapContext::HumaFinance(accounts), SwapData::HumaFinance(())) => {
                crate::huma_finance::HumaFinance::swap_signed(
                    accounts,
                    in_amount,
                    minimum_out_amount,
                    &(),
    ...
    match tag {
        ...
        SwapProtocolTag::HumaFinance => {
            #[cfg(feature = "huma-finance-swap")]
            {
                validate_tagged_program_account(
                    program_account,
                    &crate::huma_finance::HUMA_FINANCE_PROGRAM_ID,
                )?;
                let ctx = crate::huma_finance::HumaFinanceSwapAccounts::try_from(mine)?;
                Ok((SwapContext::HumaFinance(ctx), rest))
            }
            #[cfg(not(feature = "huma-finance-swap"))]
            {
                Err(ProgramError::InvalidInstructionData)
            }
        }
