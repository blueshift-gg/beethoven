use {
    crate::helper::{
        beethoven_program_path, build_swap_instruction, byreal_clmm_fixtures_dir,
        common_fixtures_dir, create_token_account, get_token_balance, load_and_set_json_fixture,
        load_program, send_transaction, setup_svm, BYREAL_CLMM_PROGRAM_ID, MEMO_PROGRAM_ID,
        TEST_PROGRAM_ID, TOKEN_2022_PROGRAM_ID, TOKEN_PROGRAM_ID,
    },
...
const SOL_USDC_POOL: Address = address!("9GTj99g9tbz9U6UYDsX6YeRTgUnkYG6GTnHv3qLa5aXq");
...
const TICK_ARRAY_BITMAP_EXTENSION: Address =
    address!("GuDQwyNv6g6jtZg13VNcDnK9mb6q8pTj534BDehqvtzF");
const TICK_ARRAY_STATE_1: Address = address!("GDQhX4LDosN5mCgTCWPqmYGRCv74QyqjLvN3JE973Wqy");
const TICK_ARRAY_STATE_2: Address = address!("EgRpnFqnu8TY4tcuPieME916qdQZTVLGhYLQ42q4oRrM");
const TICK_ARRAY_STATE_3: Address = address!("3kEBsgJenBhHLBCJ3q3uD6FnpEtfHmnPFYRSLMKENF8e");

#[test]
fn test_byreal_clmm_swap_cpi() {
    ...
    // Load Byreal CLMM program
    load_program(
        &mut svm,
        BYREAL_CLMM_PROGRAM_ID,
        &format!("{}/byreal_clmm.so", byreal_clmm_fixtures_dir()),
    );
    ...
    load_and_set_json_fixture(
        &mut svm,
        &format!("{}/amm_config.json", byreal_clmm_fixtures_dir()),
    );
    load_and_set_json_fixture(
        &mut svm,
        &format!("{}/sol_usdc_pool_state.json", byreal_clmm_fixtures_dir()),
    );
    load_and_set_json_fixture(
        &mut svm,
        &format!("{}/sol_vault.json", byreal_clmm_fixtures_dir()),
    );
    load_and_set_json_fixture(
        &mut svm,
        &format!("{}/usdc_vault.json", byreal_clmm_fixtures_dir()),
    );
    load_and_set_json_fixture(
        &mut svm,
        &format!(
            "{}/sol_usdc_observation_state.json",
            byreal_clmm_fixtures_dir()
        ),
    );
    load_and_set_json_fixture(
        &mut svm,
    ...
    load_and_set_json_fixture(
        &mut svm,
        &format!("{}/tick_array_state_1.json", byreal_clmm_fixtures_dir()),
    );
    load_and_set_json_fixture(
        &mut svm,
        &format!("{}/tick_array_state_2.json", byreal_clmm_fixtures_dir()),
    );
    load_and_set_json_fixture(
        &mut svm,
        &format!("{}/tick_array_state_3.json", byreal_clmm_fixtures_dir()),
    );

    // Jump ahead by pool open time (1_750_611_300) + 1 second
    let mut clock = svm.get_sysvar::<Clock>();
    clock.unix_timestamp = 1_750_611_300 + 1;
    ...
    // Byreal CLMM accounts layout (14 accounts + remaining accounts)
    let accounts = vec![
        AccountMeta::new_readonly(BYREAL_CLMM_PROGRAM_ID, false), // byreal_clmm_program
    ...
    match result {
        Ok(_compute_units) => {
            ...
            println!(
                "Byreal CLMM swap successful! WSOL: {} -> {}, USDC: {} -> {}",
                initial_wsol, final_wsol, initial_usdc, final_usdc
            );
        }
        Err(e) => {
            panic!("Byreal CLMM swap CPI failed: {}", e);
        }
    }
}
