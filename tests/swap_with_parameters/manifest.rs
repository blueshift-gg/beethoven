...
/// Test SwapWithParameters for Manifest using LiteSVM.
///
/// Account layout:
///   [0] user_wallet   (signer)       — payer
///   [1] in_ata                        — trader_base (SOL, selling)
///   [2] out_ata                       — trader_quote (USDC, buying)
///   --- remaining (positions 3+) ---
///   [3] manifest_program              — detector
///   [4] owner         (signer)
///   [5] market
///   [6] system_program
///   [7] base_vault
...
///   [10] token_program_quote
///   [11] base_mint
///   [12] quote_mint
///   [13] global
///   [14] global_vault
///
/// Extra data: is_exact_in (1 byte). Direction derived on-chain.
#[test]
fn test_manifest_swap_with_parameters_cpi() {
    ...
    // Build swap_with_parameters instruction: sell 0.1 SOL for USDC
    let in_amount = 100_000_000u64; // 0.1 SOL
    let min_out_amount = 1u64; // Very loose slippage for test

    // SwapWithParameters layout: [user_wallet, in_ata, out_ata, ...remaining]
    let accounts = vec![
        // Root parameters (positions 0-2)
        AccountMeta::new(payer.pubkey(), true),                // user_wallet (signer)
        AccountMeta::new(trader_base, false),                  // in_ata (SOL — selling)
        AccountMeta::new(trader_quote, false),                 // out_ata (USDC — buying)
        // Remaining (positions 3+): first is detector
        AccountMeta::new_readonly(MANIFEST_PROGRAM_ID, false), // manifest_program (detector)
    ...
    // Extra data: only is_exact_in (1 byte). No is_base_in — derived on-chain.
    let extra_data = [1u8]; // is_exact_in = true

    let instruction =
        build_swap_with_parameters_instruction(accounts, in_amount, min_out_amount, &extra_data);
    ...
    match result {
        Ok(_compute_units) => {
            ...
            println!(
                "SwapWithParameters successful! WSOL: {} -> {}, USDC: {} -> {}",
                initial_wsol, final_wsol, initial_usdc, final_usdc
            );
        }
        Err(e) => {
            panic!("SwapWithParameters CPI failed: {}", e);
        }
    }
}

#[test]
fn test_manifest_swap_with_parameters_cpi_mollusk() {
    // Load program bytes
    ...
    let account_metas = vec![
        // Root parameters (positions 0-2)
        AccountMeta::new(payer, true),                         // user_wallet (signer)
        AccountMeta::new(trader_base_addr, false),             // in_ata (SOL — selling)
        AccountMeta::new(trader_quote_addr, false),            // out_ata (USDC — buying)
        // Remaining (positions 3+): first is detector
        AccountMeta::new_readonly(MANIFEST_PROGRAM_ID, false), // manifest_program (detector)
    ...
    let instruction = build_swap_with_parameters_instruction(
        account_metas,
        in_amount,
        min_out_amount,
        &extra_data,
    ...
    println!(
        "Mollusk manifest swap_with_parameters CPI succeeded! Compute units: {}",
